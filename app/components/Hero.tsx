...
const slideData = [
  ...
  {
    id: 4,
    bgImage: "https://images.unsplash.com/photo-1497215728101-856f4ea42174?auto=format&fit=crop&q=80",
...
const Hero = () => {
  ...
  return (
    <section className="relative flex-1 flex flex-col w-full overflow-hidden text-white font-sans min-h-[calc(100vh-70px)] transition-colors duration-1000">
      ...
      <div className={`absolute inset-0 bg-gradient-to-br -z-20 transition-all duration-1000 ease-in-out 
...
<div 
  className={`absolute inset-0 z-50 transition-opacity duration-1000 
    ${isDark 
      ? "bg-[radial-gradient(circle_at_50%_50%,_rgba(99,102,241,0.15),_transparent_70%)]" 
      : "bg-[radial-gradient(circle_at_50%_50%,_rgba(255,255,255,0.3),_transparent_70%)]"
    }`} 
/>
        ...
        <div className="absolute right-6 md:right-10 top-1/2 -translate-y-1/2 flex flex-col gap-8 items-center z-30">
          {slideData.map((slide, index) => (
            <div key={slide.id} className="relative flex items-center group">
              {/* Tooltip Label */}
              <span className={`absolute right-12 whitespace-nowrap px-3 py-1 rounded-lg text-xs font-bold transition-all duration-300 opacity-0 group-hover:opacity-100 translate-x-4 group-hover:translate-x-0 backdrop-blur-md border border-white/20
        ...
        {/* HERO CONTENT AREA */}
        <div className={`flex flex-col w-full bg lg:flex-row justify-between items-center p-10 flex-grow mt-20 transition-all duration-500 ${isAnimating ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'}`}>
          ...
          <div className=" lg:text-right px-10 flex flex-col items-center lg:items-end">
            ...
            <h1 className="text-6xl md:text-8xl font-black mb-2 tracking-tighter italic">قالب</h1>
            <p className="text-yellow-400 text-lg md:text-xl tracking-[0.4em] font-bold">آية من الجمال</p>
          </div>
          ...
          <div className="relative flex items-center justify-center p-20 scale-90 md:scale-100">
            <div className="grid grid-cols-2 gap-4 rotate-45">
              {[Globe, Contact2, Briefcase].map((Icon, i) => (
                <div key={i} className={`w-28 h-28 md:w-36 md:h-36 backdrop-blur-md border border-white/10 flex items-center justify-center hover:bg-pink-600/20 hover:border-pink-500/50 transition-all cursor-pointer group ${isDark ? 'bg-slate-900/40' : 'bg-white/5'}`}>
                  <Icon size={36} className="-rotate-45 group-hover:scale-110 transition-transform" />
                </div>
              ))}
              
              <div className="w-28 h-28 md:w-36 md:h-36 bg-white text-slate-900 shadow-[0_0_50px_rgba(255,255,255,0.2)] flex items-center justify-center">
                <div className="-rotate-45 flex flex-col items-center">
                  <MapPin size={28} className="text-pink-600 animate-bounce" />
                  <p className="text-center text-[10px] md:text-xs font-black mt-1 text-pink-600">{currentSlide.mainText}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* BOTTOM NAVIGATION */}
        <div className={`w-full grid grid-cols-2 lg:grid-cols-4 mt-auto border-t border-white/10 transition-all duration-500
